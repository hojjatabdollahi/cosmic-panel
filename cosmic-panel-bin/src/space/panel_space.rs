        ...
        match self.space_event.take() {
            Some(e) => match e {
                SpaceEvent::WaitConfigure { first, mut width, mut height } => {
                        ...
                        (renderer.as_mut(), self.egl_surface.as_mut())
                    {
                        ...
                        // The panel deliberately collapses to a degenerate size to force a reconfigure
                        // but that size must not be *published* to the compositor because a too-small surface causes fatal `radius_too_large`.
                        // `render()` already refuses to draw below 20px keep the published size in lockstep with that.
            ...
            None => {
                    ...
                    (renderer.as_mut(), self.egl_surface.as_mut())
                {
                    ...
                    let _ = renderer.bind(egl_surface);
                    let scaled_size = dim.to_f64().to_physical(self.scale).to_i32_round();
                    // See note in the other resize branch (never publish a degenerate intermediate size)
                    if dim.w > 20 && dim.h > 20 {
                        egl_surface.resize(scaled_size.w, scaled_size.h, 0, 0);
                        if let Some(viewport) = self.layer_viewport.as_ref() {
